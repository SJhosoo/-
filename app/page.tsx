...
interface AnalyzeResult {
  ...
  ring: RingProduct;
  ringReason: string;
  secondRing: RingProduct;
  secondRingReason: string;
...
const LOADING_MESSAGES = [
  "두 분의 생년월일시를 살펴보는 중이에요",
  "목·화·토·금·수, 오행의 균형을 계산하는 중이에요",
  "두 분의 기운이 어떻게 어우러지는지 읽는 중이에요",
  "23종 라인업 중 어울리는 반지를 고르는 중이에요",
  "따뜻한 궁합 이야기를 다듬는 중이에요",
];
...
function LoadingScreen() {
  ...
  return (
    <div className="flex flex-col items-center gap-5 mt-24 animate-fadeUp">
      <div className="w-10 h-10 rounded-full border-2 border-[var(--panel-line)] border-t-[var(--silver)] animate-spin" />
      <p
        key={step}
        className="text-sm text-[var(--muted)] animate-fadeUp text-center max-w-xs"
      >
        ...
        {LOADING_MESSAGES.map((_, i) => (
          <span
            ...
            className={`w-1.5 h-1.5 rounded-full transition-colors duration-300 ${
              i === step ? "bg-[var(--silver)]" : "bg-[var(--panel-line)]"
            }`}
          />
        ))}
      </div>
      ...
      <section className="mb-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <RingCard
            label="1순위 추천"
            ring={result.ring}
            reason={result.ringReason}
          />
          <RingCard
            label="2순위 추천"
            ring={result.secondRing}
            reason={result.secondRingReason}
          />
        </div>
      </section>
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-[var(--panel-line)] bg-[var(--panel)]">
      <div className="relative w-full aspect-square">
        ...
        />
        <span className="absolute top-3 left-3 text-xs px-3 py-1 rounded-full bg-[var(--ink)]/80 text-[var(--moon)] border border-[var(--panel-line)]">
          {label}
        </span>
      </div>
      <div className="p-5">
        <h3 className="font-serifkr text-lg mb-1">
          {ring.name}{" "}
          <span className="text-[var(--muted)] text-sm">| {ring.nameEn}</span>
        </h3>
        <p className="text-sm leading-relaxed text-[var(--moon)] mt-2 mb-3">
          {reason}
        </p>
        <a
